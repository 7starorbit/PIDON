...
class ConvBlock3D(nn.Module):
    def __init__(self, in_ch, out_ch, kernel_size=3, stride=1, padding=1):
        ...
        self.bn1 = nn.GroupNorm(num_groups=min(8, out_ch), num_channels=out_ch)
        self.bn2 = nn.GroupNorm(num_groups=min(8, out_ch), num_channels=out_ch)
        self.gelu = nn.GELU()
        if in_ch != out_ch:
            self.shortcut = nn.Sequential(nn.Conv3d(in_ch, out_ch, kernel_size=1, stride=1, padding=0), nn.GroupNorm(num_groups=min(8, out_ch), num_channels=out_ch))
        else:
